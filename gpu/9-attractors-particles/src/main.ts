...
async function init() {
  ...
  for (let i = 0; i < attractorsPositions.array.length; i++) {
    ...
    attractor.position = attractorsPositions.array[i] as THREE.Vector3;
  ...
  gui
    .addColor({color: colorA.value.getHexString(THREE.SRGBColorSpace)}, "color")
    .name("colorA")
    .onChange((value) => colorA.value.set(value));
  gui
    .addColor({color: colorB.value.getHexString(THREE.SRGBColorSpace)}, "color")
    .name("colorB")
